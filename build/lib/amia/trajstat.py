...
class TrajStat:
    ...
    def _ionic_interaction_metrics(self, u, cations, anions, output_prefix,
                                   cutoff=6.0):
        """Measure contact ion pairs as unique residue pairs within 6 A."""
        frame_rows = []
        pair_distances = {}
        n_frames = 0
        for ts in u.trajectory:
            n_frames += 1
            distances = contacts.distance_array(cations.positions, anions.positions)
            present_pairs = set()
            for cation_index, anion_index in np.argwhere(distances < cutoff):
                ...
                distance = float(distances[cation_index, anion_index])
                present_pairs.add(pair)
                pair_distances.setdefault(pair, []).append(distance)
            frame_rows.append([ts.frame, ts.time / 1000.0, len(present_pairs)])

        frame_df = pd.DataFrame(
            frame_rows,
            columns=['Frame', 'Time (ns)', 'Ionic contacts (<6 A)'],
        )
        frame_df.to_csv(output_prefix + '_ionic_interactions.csv', index=False)
        ...
        pair_df.to_csv(output_prefix + '_ionic_pair_occupancy.csv', index=False)
        return frame_df
    ...
    def saltbridges(self, top_file, traj_file):
        ...
        ca_df = ca_df.rename(columns={'Ionic contacts (<6 A)': str(top_file).split('.')[0]})
    ...
    def _write_hbond_metrics(self, hbonds, output_prefix, count_column):
        """Write the documented HydrogenBondAnalysis metrics for one analysis."""
        timeseries = pd.DataFrame({
            'Time (ns)': np.asarray(hbonds.times) / 1000.0,
            count_column: hbonds.count_by_time(),
        })
        timeseries.to_csv(output_prefix + '_hbonds.csv', index=False)
        ...
        observations.to_csv(output_prefix + '_hbonds_observations.csv', index=False)

        by_id = pd.DataFrame(
            hbonds.count_by_ids(),
            columns=['Donor index', 'Hydrogen index', 'Acceptor index', 'Count'],
        )
        by_id.to_csv(output_prefix + '_hbonds_by_id.csv', index=False)

        by_type = pd.DataFrame(
            hbonds.count_by_type(),
            columns=['Donor resname', 'Donor type', 'Acceptor resname',
                     'Acceptor type', 'Count'],
        )
        by_type.to_csv(output_prefix + '_hbonds_by_type.csv', index=False)
        ...
        lifetime.to_csv(output_prefix + '_hbonds_lifetime.csv', index=False)
        return timeseries
    ...
    def hbond_calc(self, top_file, traj_file, start_fr):
        ...
        for seg in u.segments:
            if 'PRO' not in str(seg) and str(seg)[1:-1].split(' ')[1].split('_')[2] not in solvent_mol and \
                ...
                output_prefix = str(seg)[1:-1].split(' ')[1].split('_')[2]
                df = self._write_hbond_metrics(
                    hbonds, output_prefix, str(top_file).split('.')[0]
                )
