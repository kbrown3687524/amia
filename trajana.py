...
class TrajAna:
    ...
    def rmsd_calc(self, traj_file, top_file):
        ...
        dataframe = pd.DataFrame(R.results.rmsf, columns=['Frame','Time (ns)', str(traj_file)])
    ...
    def rmsd_comp(self, traj_folder, output_dir):
        ...
        for folder in os.listdir(traj_folder):
            ...
            rmsd_df.to_csv(str(folder) + '.csv')
            if df.empty:
                df = pd.concat([df, rmsd_df], axis=1)
            else:
                df = pd.concat([df, rmsd_df.iloc[:, -1]], axis=1)
        os.chdir(output_dir)
