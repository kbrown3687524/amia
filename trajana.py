...
class TrajAna:
    ...
    def pca_calc(self, top_file, traj_file):
        u = mda.Universe(top_file, traj_file)
        pc = pca.PCA(u, select='backbone', align=True, mean=None,
                             n_components = None, verbose=True).run(verbose=True, step=10)
        print('backbone')
    ...
    def pca_comp(self, traj_folder):
        for folder in os.listdir(traj_folder):
            top_file = ''
            traj_file = ''
            os.chdir('{0}/{1}'.format(str(traj_folder), folder))
            for file in os.listdir():
                if file.endswith('.xtc') and 'rmsfit_' in str(file):
                    traj_file = file
                elif file.endswith('.tpr'):
                    top_file = file
            pca_df = self.pca_calc(top_file, traj_file)
            pca_df.to_csv(str(top_file).split('.')[0] + '_PCA_2.csv')

    def pca_plotting(self, traj_folder):
        for folder in os.listdir(traj_folder):
            top_file = ''
            traj_file = ''
            os.chdir('{0}/{1}'.format(str(traj_folder), folder))
            for file in os.listdir():
                if file.endswith('_PCA.csv'):
                    df = pd.read_csv(file)
                    print(df)
                    g = sns.PairGrid(df.iloc[:,1:], hue='Time (ps)', palette=sns.color_palette('Oranges_d', n_colors=len(df)))
...
def main(systems, output_dir):
    ...
    #p1 = multiprocessing.Process(target = p.rmsd_comp(systems, output_dir))
    #p2 = multiprocessing.Process(target = p.rmsf_comp(systems, output_dir))
    #p3 = multiprocessing.Process(target = p.rgyr_comp(systems, output_dir))
    p4 = multiprocessing.Process(target = p.pca_comp(systems))
    #p1.start()
    #p2.start()
    #p3.start()
    p4.start()
