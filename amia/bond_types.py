...
class Contacts:
    ...
    def saltbridges_comp(self, traj_folder, output_dir):
        ...
        for folder in os.listdir(traj_folder):
            top_file = ''
            traj_file = ''
            os.chdir(os.path.join(str(traj_folder), folder))
    ...
    def multi_sb_plotting(self, outputs_dir, start_fr):
        ...
        for file in os.listdir(outputs_dir):
            if file.endswith('multi_variant_saltbridges.csv'):
                ...
                plt.ylabel(r"$N_{II}$")
                plt.savefig(os.path.join(str(outputs_dir), str(file).split('.')[0] + '.tiff'),
    ...
    def dna_prot_hbonds(self, top_file, traj_file, start_fr):
        u = mda.Universe(traj_file, top_file)
        hbonds_df = pd.DataFrame()
        df = pd.DataFrame()
    ...
    def hbond_comp(self, traj_folder, output_dir, start_fr):
        ...
        for folder in os.listdir(traj_folder):
            ...
            sys_dna_hbonds = self.dna_prot_hbonds(traj_file, top_file, start_fr)
    ...
    def multi_hbonds_plots(self, outputs_dir, start_df):
        ...
        for system in os.listdir(systems):
            top_file = ''
            traj_file = ''
            os.chdir(os.path.join(str(systems), system))
            for file in os.listdir():
                if file.endswith('.xtc') and 'rmsfit_' not in str(file):
                    traj_file = file
                elif file.endswith('.tpr'):
            ...
            u = mda.Universe(top_file, traj_file)
            for seg in u.segments:
                if 'PRO' not in str(seg) and str(seg)[1:-1].split(' ')[1].split('_')[2] not in solvent_mol and str(seg)[1:-1].split(' ')[1].split('_')[2] not in ions and 'DNA' not in str(seg):
            ...
            for system in os.listdir(systems):
                ...
                for sys_file in os.listdir():
                    if str(hetatm) in str(sys_file) and 'hbonds.csv' in str(sys_file):
                         hetatm_file_pair.setdefault(hetatm, []).append(os.path.join(os.getcwd(), sys_file))
        print(hetatm_file_pair)
        for key in hetatm_file_pair:
            ...
            plt.ylabel(r"$N_{HB}$")
            plt.legend(bbox_to_anchor=(1.0, 1.0), loc='upper left')
            plt.savefig(os.path.join(str(output_dir), 'Multi-system Protein-' + str(key) + '_hbonds.tiff'), bbox_inches='tight', dpi=900)

    def multi_DNA_hbonds_plots(self, outputs_dir, start_df):
        """This function plots the 'multi-variant_hbonds'.csv file generated by the hbonds_comp function from within the
        previously defined output directory and creates a figure with each dataset present within the file. This
        function was developed prior to the single plot generation, however too many datasets resulted in degraded
        visualization."""
        ...
        for system in os.listdir(systems):
            os.chdir(os.path.join(str(systems), system))
            for file in os.listdir():
                if str('DNA') in str(file) and 'hbonds.csv' in str(file):
                    dna_hbonds_df = pd.read_csv(file)
                    dna_hbonds_df['Time (ns)'] = dna_hbonds_df['Time (ns)']/1000
                    if df.empty == True:
                        df = dna_hbonds_df.iloc[:,1:]
                    else:
                        df = pd.concat([df, dna_hbonds_df.iloc[:,2:]], axis=1)
            os.chdir(systems)
        df.plot(x='Time (ns)', linewidth=0.5)
        plt.title('Multi-system Protein-DNA H-bonds')
        plt.ylabel(r"$N_{HB}$")
        plt.legend(bbox_to_anchor=(1.0, 1.0), loc='upper left')
        plt.savefig(os.path.join(str(output_dir), 'Multi-system Protein-DNA_hbonds.tiff'), bbox_inches='tight', dpi=900)
    ...
    def hetatm_calc(self, top_file, traj_file, start_fr):
        ...
        u = mda.Universe(top_file, traj_file)
        for seg in u.segments:
            if 'PRO' not in str(seg) and str(seg)[1:-1].split(' ')[1].split('_')[2] not in solvent_mol and 'DNA' not in str(seg):
    ...
    def hetatm_plot(self, systems, outputs_dir, start_fr):
        ...
        for folder in os.listdir(systems):
            ...
            for file in os.listdir():
                if file.endswith('hetatm_contacts.csv'):
                    system_hetatm_conts.setdefault(folder, []).append(os.path.join(os.getcwd(), file))
            os.chdir(systems)
        ...
        for i in range(contact_sys):
            ...
            plt.ylabel(r"$N_{II}$")
            plt.legend(bbox_to_anchor=(1.0, 1.0), loc='upper left')
            plt.savefig(os.path.join(outputs_dir, 'Multi-system ' + str(title_hetatm[0]) + str(title_hetatm[1]) + ' interactions.tiff'), bbox_inches='tight', dpi=900)

    def dna_saltbridges_calc(self, top_file, traj_file):
        """Saltbridges or ionic bonds occur between acidic and basic residues within the protein structure. Here the
        residues and the respective atoms involved are selected and passed to the contacts within cutoff function,
        and returned as a dataframe which is then stored within the original system folder as a .csv file
        for figure generation."""
        ...
        ca_df = pd.DataFrame(ca, columns=['Time (ns)', str(top_file).split('.')[0]])
        ca_df['Time (ns)'] = ca_df['Time (ns)'] / 100
        ca_df.to_csv(str(top_file.split('.')[0] + '_DNA_saltbridges.csv'))
        print(ca_df)

    def dna_ionic_conts(self, traj_folder):
        for folder in os.listdir(traj_folder):
            top_file = ''
            traj_file = ''
            os.chdir(os.path.join(str(traj_folder), folder))
            for file in os.listdir():
                if file.endswith('.xtc') and 'rmsfit_' not in str(file):
                    ...
                    top_file = file
            print(top_file, traj_file)
            self.dna_saltbridges_calc(top_file, traj_file)

    def ionic_dna_plot(self, systems, outputs_dir, start_fr):
        os.chdir(systems)
        multi_df = pd.DataFrame()
        for folder in os.listdir(systems):
            os.chdir(folder)
            for file in os.listdir():
                if file.endswith('DNA_saltbridges.csv'):
                    heading = str(file).split('.')[0].split('_')
                    title = heading[0] + ' ' + heading[1] + '-' + heading[2]
                    ionic_dna_df = pd.read_csv(file)
                    ionic_dna_df = ionic_dna_df.iloc[int(start_fr):, 1:]
                    if multi_df.empty == True:
                        multi_df = ionic_dna_df
                    else:
                        multi_df = pd.concat([multi_df, ionic_dna_df.iloc[:,1]], axis=1)
            os.chdir(systems)
        multi_df.plot(x='Time (ns)', linewidth=0.5)
        plt.legend(bbox_to_anchor=(1.0, 1.0), loc='upper left')
        plt.ylabel(r"$N_{II}$")
        plt.title('Multi-system Protein-DNA Ionic Interactions')
        plt.savefig(os.path.join(outputs_dir, 'Multi-system Protein-DNA Ionic Interactions.tiff'), bbox_inches='tight', dpi=900)
...
def main(systems, output_dir, start_fr):
    ...
    # p.multi_DNA_hbonds_plots(output_dir, start_fr)
    #p.hetatm_conts(systems, start_fr)
    p.hetatm_plot(systems, output_dir, start_fr)
    # p.dna_ionic_conts(systems)
    #p.ionic_dna_plot(systems, output_dir, start_fr)
